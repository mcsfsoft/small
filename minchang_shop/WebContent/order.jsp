...
<script type="text/javascript">
	$(function(){
		$("#btn_add").click(function(){
			location.href = "orders?action=addOrder&aid="+$("#address").val();
		})
	})
</script>
</head>
...
<div class="container" style="background-color: white;">
	...
	<div class="row" style="margin-top: 40px;">
	 ...
	 <!-- 价格 -->		<th>${cart.value.goods.price}</th>
	 					<th>${cart.value.num}</th>
	 					<th>${cart.value.num*cart.value.goods.price}</th>
	 				
	 				<c:set var="sum" value="${cart.value.goods.price*cart.value.num +sum }"></c:set>
	 				</tr>
	 			</c:forEach>	
 				...
 				<tr>
 				 <td colspan="5">
 				 	...
 				 	<c:forEach var="ad" items="${requestScope.adresses}">
 				 		<option value="${ad.id}"
 				 		<c:if test="${ad.level==0}"> 
 				 		 	selected
 				 		 </c:if>
 				 			>${ad.detail}
 				 		 </option>
